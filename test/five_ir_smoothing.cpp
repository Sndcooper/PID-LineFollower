...
void forward(int rightSpeed, int leftSpeed){
  ...
  float diffLeft = leftSpeed - currentLeft;
  if (abs(diffLeft) <= threshold) {
    currentLeft = leftSpeed; 
  }
  else {
    if (diffLeft > 0) currentLeft += smoothStep;
    else currentLeft -= smoothStep;
  }

  float diffRight = rightSpeed - currentRight;
  if (abs(diffRight) <= threshold) {
    currentRight = rightSpeed;
  }
  else {
    if (diffRight > 0) currentRight += smoothStep;
    else currentRight -= smoothStep;
  }
  delay(4);
...
void loop() {
  ...
  if (s0 && s1 && s3)
  {
    ...
    if (s3) forward(turnspeed, -turnspeed);
    prev = -2;
  } else if(s2){
    forward(dampSharp, speed + turndelta);
    if (s4) forward(-turnspeed, turnspeed);
    prev = 2;
  } else if(s0){
    forward(speed, speed);
    prev = 0;
